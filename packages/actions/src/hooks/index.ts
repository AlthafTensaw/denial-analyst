...
export function useActionQuery<TData = unknown>(
  ...
  // Derive a stable string key from the raw request to prevent infinite loops.
  const rawRequestKey = useMemo(
    () => resolveCacheKey(actionId, request),
    [actionId, request],
  );

  // Pre-validate so cache key derivation matches the dispatcher's validated
  // request shape (Issue #1). Memoized on rawRequestKey.
  ...
  }, [actionId, rawRequestKey]);

  // The final cache key must use the validated request (with defaults)
  // to match the dispatcher's write key.
  const cacheKey = useMemo(
    () => resolveCacheKey(actionId, validatedRequest),
