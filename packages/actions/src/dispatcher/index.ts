...
import { getAction, getInvalidationsFor } from '../registry';
import type {
  ActionDeclaration,
  ActionError,
  ActionResult,
...
export function readCacheValue<T = unknown>(cacheKey: string): T | undefined;
export function readCacheValue<T = unknown>(actionId: string, request: unknown): T | undefined;
export function readCacheValue<T = unknown>(
  ...
  if (request === undefined) {
    ...
    if (sep < 0) {
      return queryClient.getQueryData<T>(['action', compositeKey, ''] as const);
    }
    const actionId = compositeKey.slice(0, sep);
    const requestKey = compositeKey.slice(sep + 2);
    return queryClient.getQueryData<T>(buildActionQueryKey(actionId, requestKey));
  }
  return queryClient.getQueryData<T>(actionQueryKey(actionIdOrKey, request));
}
 ...
 * `packages/runtime/docs/ADR-001-redux-to-zustand.md`.
 */
export function subscribeToCacheKey(cacheKey: string, fn: (value: unknown) => void): () => void {
  ...
  if (sep < 0) {
    ...
    queryKey = buildActionQueryKey(cacheKey.slice(0, sep), cacheKey.slice(sep + 2));
  }
  ...
  return cache.subscribe((event) => {
    ...
    if (eventKey.length === queryKey.length && eventKey.every((part, i) => part === queryKey[i])) {
  ...
  if (decl.kind === 'query' || decl.kind === 'mutation') {
    ...
    const dryRunData: DryRunResult = {
      ...
      wouldDispatch: {
        method,
        url,
        ...(method === 'GET' || method === 'DELETE' ? {} : { body: JSON.stringify(remainder) }),
      },
    };
...
async function executeMutation<T>(
  ...
  if (decl.optimistic && decl.optimistic.pattern !== 'none') {
    ...
    for (const q of allQueries) {
      ...
      if (cachedDecl?.kind !== 'query' || cachedDecl.cache?.tag !== targetTag) {
        continue;
      }
  ...
  if (!adapter) {
    return Promise.resolve({
      ...
      error: {
        ...
        message: 'No router adapter configured. Call setRouterAdapter(...) during app boot.',
      },
    });
  }
  const target = decl.to(request);
...
async function runHttpToData(
  ...
  try {
    ...
    throw new ApiError('BAD_REQUEST', 0, e instanceof Error ? e.message : String(e));
  }
  const { method, path, remainder } = resolved;
