...
export async function authenticatedFetch<T = unknown>(
  ...
  // 1.5 Special case for error with 'detail' (common in FastAPI/Python backends)
  // If the status is not OK but the body contains a 'detail' object that
  // matches our expected schema, we treat it as success.
  if (
    !response.ok &&
    body &&
    typeof body === 'object' &&
  ...
  // 3. Fallback: Direct JSON response (no envelope).
  // This supports the Operations Console staging backend which returns
  // unwrapped data. We validate the body directly against the schema.
  const directParse = dataSchema.safeParse(body);
  if (directParse.success) {
    return directParse.data as T;
  }

  // 4. Failure: Doesn't match envelope OR direct schema.
  throw new ApiError(
    'PLATFORM_ENVELOPE_INVALID',
    response.status,
    'Response did not match expected envelope or schema',
    {
      envelopeError: successParse.error.flatten(),
      directError: directParse.error.flatten(),
    },
  );
}
