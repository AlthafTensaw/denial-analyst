...
export function WorkflowStepsList({
  ...
  const stepComplete = useActionMutation('denial.step-complete');
  ...
  const handleToggle = async (step: WorkflowStep) => {
    ...
    setError(null);
    try {
      const response = (await stepComplete.mutateAsync({
        ...
        {steps.map((step) => {
          ...
          return (
            <li
              ...
              style={{
                ...stepStyle,
                background: isNext ? 'var(--tw-color-teal-50)' : undefined,
                borderColor: isNext
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={stepMetaStyle}>
                  ...
                  {done && step.completed_by && (
                    <>
                      {' · '}
                      <span style={{ color: 'var(--tw-color-teal-700)' }}>
                        ✓ {step.completed_by}
                        {step.completed_at &&
