...
export function RowDetailPanel({
  ...
  const claimDetail = useActionQuery('denial.claim-detail', {
    claim_id: claim.claim_id,
  });
  const denialEvents = useActionQuery('denial.denial-events', {
    claim_id: claim.claim_id,
  });

  const accept = useActionMutation('denial.accept');
  const override = useActionMutation('denial.override');
  const complete = useActionMutation('denial.complete');
  ...
  const handleAccept = async () => {
    try {
      await accept.mutateAsync({ classification_id: c.classification_id });
  ...
  const handleComplete = async () => {
    try {
      await complete.mutateAsync({ classification_id: c.classification_id });
  ...
  return (
    <div style={containerStyle}>
      ...
      {claimDetail.isError && !claimDetail.data && (
        <ClaimDetailHeader
          detail={null as never}
          classificationId={c.classification_id}
      ...
      <section style={regionStyle}>
        ...
        <DenialEventsList
          ...
          error={denialEvents.isError ? denialEvents.error : undefined}
        />
      </section>
      ...
      {canAct && (
        <div style={actionBarStyle}>
          ...
          {isWorkedOutsideTool && canAcceptOrOverride && (
            <>
              <button
                ...
                disabled={override.isPending || complete.isPending}
              >
          ...
          <RunClassifierNowButton
            ...
            onClassified={onMutationComplete}
          />
...
const containerStyle: React.CSSProperties = {
  padding: 14,
  background: 'var(--tw-color-bg-secondary, white)',
  display: 'grid',
  gap: 14,
};
...
const primaryButtonStyle: React.CSSProperties = {
  background: 'var(--tw-color-teal-600, #149A9A)',
  color: 'white',
  border: 'none',
  padding: '8px 14px',
  borderRadius: 6,
...
const toastStyle: React.CSSProperties = {
  ...
  background: 'var(--tw-color-gray-900, #111)',
  color: 'white',
  borderRadius: 6,
  fontSize: '0.8125rem',
};
